...
public class AudioValores {
    ...
    public void audio_Centena(double valor, String Diretorio) throws Exception{
        ...
        /*Concatena o diretório onde estão os arquivos de aúdio, 
        nome do arquivo localizado no enumerador e extensão .wav*/
        if(i_dezena == 0 && i_unidade == 0){
        play.executaSom(Diretorio+"Cem.wav",false);
        }
        else{
    ...
    public void audio_descricao(int opcao, String Diretorio) throws Exception{ //Executta o áudio inicial de cada opção
        ExecutaSom play = new ExecutaSom();
        switch(opcao){
            case 1:
                play.executaSom(Diretorio+"ValorFatura.wav",false);
                break;
            case 2:
                play.executaSom(Diretorio+"LimiteCartao.wav",false);
                break;
            case 3:
                play.executaSom(Diretorio+"SaldoCompras.wav",false);
                break;
        }
    }
}